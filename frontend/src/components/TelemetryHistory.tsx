// components/TelemetryHistory.tsx
...
const TelemetryHistory: React.FC<TelemetryHistoryProps> = ({ 
  robotId, 
  history, 
  lastUpdated,
  isAutoRefresh,
  ...
  return (
    <div className="p-6 space-y-6" style={{ backgroundColor: theme.colors.background.main }}>
      ...
      <div className="flex justify-between items-start flex-wrap gap-4">
        <div>
          <h2 className={theme.typography.heading.h2}>{`Telemetry History for ${robotId}`}</h2>
          <div className="flex items-center gap-3 mt-2">
            ...
            <div className="flex items-center gap-2">
              <div 
                className={`w-2 h-2 rounded-full ${isAutoRefresh ? 'animate-pulse' : ''}`}
                style={{ backgroundColor: isAutoRefresh ? theme.colors.status.good.main : theme.colors.text.tertiary }}
              />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          ...
          <p className="text-sm" style={{ color: getStatusStyles("NORMAL").text }}>
            {((statusStats.NORMAL / filteredHistory.length) * 100).toFixed(1)}% of time
          </p>
        </div>
        ...
        >
          ...
          <p className="text-sm" style={{ color: getStatusStyles("WARNING").text }}>
            {((statusStats.WARNING / filteredHistory.length) * 100).toFixed(1)}% of time
          </p>
        </div>
        ...
        >
          ...
          <p className="text-sm" style={{ color: getStatusStyles("CRITICAL").text }}>
            {((statusStats.CRITICAL / filteredHistory.length) * 100).toFixed(1)}% of time
          </p>
        </div>
      </div>
          ...
          <LineChart data={filteredHistory}>
            ...
            <Tooltip 
              ...
              formatter={(value: any, name: string) => {
                ...
                return [value, metrics.find(m => m.key === name)?.label];
              }}
              contentStyle={{ 
            ...
            <Line 
              ...
              dot={{ r: 3 }}
            />
            {/* Status markers as reference lines */}
            {statusChanges.map((change, idx) => (
              <ReferenceLine
                key={idx}
                x={change.timestamp}
  ...
  >
    <div className="text-sm" style={{ color: theme.colors.text.secondary }}>
      Showing {(currentPage - 1) * PAGE_SIZE + 1} to {Math.min(currentPage * PAGE_SIZE, filteredHistory.length)} of {filteredHistory.length} records
    </div>
    <div className="flex gap-2">
